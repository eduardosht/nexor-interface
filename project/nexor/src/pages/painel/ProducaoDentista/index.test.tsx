...
function readSourceFiles(root: string): string[] {
  return readdirSync(root).flatMap((entry) => {
    ...
    return /\.(test|spec)\.(ts|tsx)$/.test(entry) ? [] : /\.(ts|tsx)$/.test(entry) ? [readFileSync(path, 'utf8')] : [];
  });
}
...
vi.mock('react-leaflet', () => ({
  MapContainer: ({ children }: { children: React.ReactNode }) => <div data-testid="provider-map">{children}</div>,
  TileLayer: () => <div data-testid="provider-tiles" />,
  useMap: () => ({
    fitBounds: vi.fn(),
    setView: vi.fn(),
  }),
  Marker: ({
    ...
    <button type="button" data-testid="provider-marker" onClick={() => eventHandlers?.click?.()}>
      {children}
    </button>
  ),
  ...
  return sharedIntake({
    ...
    payload: {
      ...
      dentist: {
        ...
        professionalObservations: 'Resumo clínico completo.',
      },
    },
    ...overrides,
  });
...
function configureApiGet({
  ...
  mockApiGet.mockImplementation((url: string) => {
    ...
    if (url === `/v1/orders/${configuredOrderId}`) {
...
function createProductionRequestDraft(overrides: Partial<ProductionRequestDraft> = {}): ProductionRequestDraft {
  return {
    ...
    scan3dFileRef: null,
    lgpdConfirmed: true,
    externalProductionProviderId: 'external-provider-edu',
      ...
      <ThemeProvider theme={lightTheme}>
        ...
        <button
          ...
          disabled={
            ...
            !draft.externalProductionProviderId ||
            !draft.lgpdConfirmed
          }
        >
          Finalizar
...
async function ensureAnamnesisSummaryStep() {
  ...
  await waitFor(
    () => {
      expect(
        screen.queryByText(/etapa 1 de 3/i) ??
          screen.queryByText(/etapa 2 de 3/i) ??
          screen.queryByText(/etapa 3 de 3/i)
      ).toBeTruthy();
    },
    { timeout: 5000 }
  );

  if (screen.queryByText(/etapa 3 de 3/i)) {
    return;
  }

  if (screen.queryByText(/etapa 2 de 3/i)) {
...
function fillRequiredDentistComplement() {
  ...
  fireEvent.click(screen.getByRole('option', { name: /linha impact/i }));
  ...
  fireEvent.change(screen.getByLabelText(/observações profissionais/i), {
...
async function fillProductionRequestUntilReadyToComplete() {
...
describe('ProducaoDentista', () => {
  ...
  it('keeps professional observations inside the dentist complement before the anamnesis summary step', () => {
    const anamnesisSource = readFileSync(join(process.cwd(), 'src/pages/painel/ProducaoDentista/DentalAnamnesisRecord.tsx'), 'utf8');
    const sharedIntakeSource = readFileSync(join(process.cwd(), 'src/pages/painel/components/sharedIntakeDefinition.ts'), 'utf8');
    ...
    expect(sharedIntakeSource).toContain("key: 'professionalObservations'");
    expect(sharedIntakeSource).toContain("label: 'Observações profissionais'");
    expect(anamnesisSource).not.toContain('<DebouncedTextArea');
    expect(anamnesisSource).not.toContain('onCommit={onSummaryChange}');
    ...
    expect(productionRequestFieldsSource).toContain('const opsNotesField = useDebouncedDraftText');
    ...
    expect(productionRequestFieldsSource).not.toContain('onChange={(event: FieldChangeEvent) => onChange({ opsNotes: event.target.value })}');
  });
  ...
  it('restricts production upload fields to a scan-only LGPD-reviewed policy', () => {
    const productionRequestFieldsSource = readFileSync(
      join(process.cwd(), 'src/pages/painel/ProducaoDentista/ProductionRequestFields.tsx'),
      'utf8'
    );

    expect(productionRequestFieldsSource).toContain('accept={PRODUCTION_UPLOAD_POLICIES.scan3d.accept}');
    expect(productionRequestFieldsSource).toContain('Formatos aceitos: STL, PLY, OBJ, DICOM, ZIP ou PDF');
    expect(productionRequestFieldsSource).toContain('100 MB.');
    expect(productionRequestFieldsSource).not.toContain('PRODUCTION_UPLOAD_POLICIES.prescription');
    expect(productionRequestFieldsSource.toLowerCase()).not.toContain('prescri');
  });
  ...
  it('keeps form action buttons on the onboarding button pattern and admin actions on the design-system Button', () => {
    ...
    expect(productionSource).not.toContain('handleSearchLabs');
    ...
    expect(workflowFormsStyles).toContain('grid-template-columns: repeat(2, minmax(0, 1fr));');
  ...
  it('keeps administrative form typography aligned with the onboarding scale', () => {
    ...
    [
      readFileSync(join(process.cwd(), 'src/pages/painel/admin/AdminDentistLicensing.tsx'), 'utf8'),
      readFileSync(join(process.cwd(), 'src/pages/painel/ProducaoDentista/index.tsx'), 'utf8'),
      readFileSync(join(process.cwd(), 'src/pages/painel/ProducaoDentista/ProductionRequestFields.tsx'), 'utf8'),
      ...readSourceFiles(join(process.cwd(), '../packages/design-system/src/components')),
    ].forEach((source) => {
      const sourceWithAllowedTinyMetadataRemoved = source
        .replace(/const PartnerEmail = styled\.span`[\s\S]*?`;/g, '')
        .replace(/const CompactModalDetailValue = styled\(AdminModalDetailValue\)`[\s\S]*?`;/g, '')
        .replace(/const CompactModalDetailLabel = styled\(AdminModalDetailLabel\)`[\s\S]*?`;/g, '');
  ...
  it('renders the dentist production wizard with three steps and blocks completion until all required data exists', async () => {
    configureApiGet();
    ...
    await fillProductionRequestUntilReadyToComplete();
    expect(screen.getByRole('button', { name: /finalizar/i })).toBeEnabled();
    expect(screen.queryByTestId('provider-map')).not.toBeInTheDocument();
  }, 10_000);
  ...
  it('shows the Biteplaner order information in the anamnesis step', async () => {
    ...
    expect(screen.getAllByText(/^linha impact$/i).length).toBeGreaterThan(0);
  ...
  it('hydrates the production request step from the latest production_request form saved in the database', async () => {
      ...
      if (path === '/v1/orders/BP-DEMO-004/forms/form-production-1') {
        return Promise.resolve({
          ...
          payload: {
            ...
            opsNotes: 'Observações salvas para a operação Nexor.',
    ...
    expect(screen.getByDisplayValue('Observações salvas para a operação Nexor.')).toBeInTheDocument();
    expect(screen.getByText(/scan-salvo\.stl/i)).toBeInTheDocument();
    expect(screen.getByText(/pedido biteplaner confirmado pelo dentista/i)).toBeInTheDocument();
    expect(screen.getByText(/^preto$/i)).toBeInTheDocument();
    expect(screen.getByText(/^2$/i)).toBeInTheDocument();
    expect(screen.getByText(/o dentista alterou o pedido antes do pagamento/i)).toBeInTheDocument();
  });
  ...
  it('opens directly on the anamnesis summary when the anamnesis was already saved', async () => {
    ...
    expect(await screen.findByText(/etapa 2 de 3/i)).toBeInTheDocument();
    expect(await screen.findByTestId('dental-anamnesis-record')).toBeInTheDocument();
    expect(screen.getAllByText(/resumo clínico completo/i).length).toBeGreaterThan(0);
    expect(screen.queryByTestId('workflow-forms-panel')).not.toBeInTheDocument();
  });
  ...
  it('opens directly on the anamnesis summary when the dentist already submitted the initial evaluation', async () => {
    ...
    expect(screen.getByRole('button', { name: /aguardando pagamento do dentista/i })).toBeDisabled();
  });
  ...
  it('allows the dentist to continue production after payment releases dentist forms', async () => {
    configureApiGet({
      order: createOrder({
        id: 'BP-DEMO-004',
        status: 'awaiting_dentist_forms',
        statusLabel: 'Aguardando documentação do dentista',
        stage: 'awaiting_dentist_forms',
        productionRequestDraft: null,
      }),
      forms: [reviewedSharedIntake()],
    ...
    expect(await screen.findByTestId('dental-anamnesis-record')).toBeInTheDocument();
    expect(getWizardNextButton()).toBeEnabled();
    expect(screen.queryByRole('button', { name: /aguardando pagamento do dentista/i })).not.toBeInTheDocument();
  });

  it('allows the dentist to resend production after external production requests an adjustment', async () => {
    configureApiGet({
      order: createOrder({
        id: 'BP-DEMO-004',
        ...
        productionRequestDraft: {
          ...
          opsNotes: 'Revisar escaneamento solicitado pela operação Nexor.',
  ...
  it('shows the initial evaluation step again when the submitted complement belongs to a previous dentist', async () => {
    ...
    expect(await screen.findByText(/etapa 1 de 3/i)).toBeInTheDocument();
  ...
  it('stops the dentist at the anamnesis summary after the initial evaluation is completed', async () => {
    ...
    expect(screen.getByText(/o dentista precisa concluir o pagamento/i)).toBeInTheDocument();
  });
  ...
  it('keeps the clinical form progress inside the form card instead of the production shell', async () => {
    ...
    const progressRegion = await screen.findByRole('region', { name: /seu progresso/i });
    expect(progressRegion).toBeInTheDocument();
    expect(within(progressRegion).queryByText(/seu cuidado/i)).not.toBeInTheDocument();
    const workflowFormsStyles = readFileSync(
      join(process.cwd(), 'src/pages/painel/components/WorkflowFormsPanel.styles.ts'),
      'utf8'
    );
    expect(workflowFormsStyles).toContain('grid-auto-flow: column;');
  ...
  it('fills professional observations in the dentist complement and shows them read-only in the anamnesis record', async () => {
    configureApiGet({ forms: [sharedIntake()] });
    ...
    mockApiPost.mockResolvedValueOnce(
      reviewedSharedIntake({
        payload: {
          customer: sharedIntake().payload.customer,
          dentist: {
            professionalObservations: 'Resumo clínico completo.',
    ...
    const professionalObservations = screen.getByLabelText(/observações profissionais/i);
    fireEvent.change(professionalObservations, {
      target: { value: 'Resumo clínico preenchido no complemento.' },
    });
    ...
    await screen.findByTestId('dental-anamnesis-record');
    expect(screen.queryByLabelText(/resumo da avaliação inicial \/ anamnese/i)).not.toBeInTheDocument();
    expect(screen.getAllByText('Resumo clínico completo.').length).toBeGreaterThan(0);
  }, 15000);

  it('requires the intraoral scan before enabling completion', async () => {
    configureApiGet();
  ...
  it('requires purchase divergence confirmation before completing the production request', async () => {
    configureApiGet({
      order: createOrder({
        purchaseConfiguration: { productKey: 'biteplaner', quantity: 2, model: 'impacto', color: 'preto' },
        dentistRecommendedPurchaseConfiguration: {
    ...
    await waitFor(() => expect(screen.getByRole('button', { name: /finalizar/i })).toBeDisabled());
  ...
  it('keeps the dentist production request as the final step without requiring external provider selection', async () => {
    configureApiGet();
    renderPage();
    ...
    fireEvent.change(screen.getByLabelText(/observações para a operação Nexor/i), {
    ...
    expect(screen.getByText(/etapa 3 de 3/i)).toBeInTheDocument();
    await waitFor(() => expect(screen.getByRole('button', { name: /finalizar/i })).toBeEnabled());
  ...
  it('submits the production request successfully when all required fields are filled', async () => {
    ...
    mockApiPost.mockImplementation((url: string, payload?: Record<string, unknown>) => {
      if (url === '/v1/orders/BP-DEMO-004/clinical-evaluation') {
        return Promise.resolve({
          order: createOrder({
            status: 'external_production_processing',
            statusLabel: 'Em revisão operacional Nexor',
            stage: 'external_production',
          }),
        });
      }
  ...
  it('does not expose external supplier selection to the dentist in the MVP production request', async () => {
    configureApiGet();
    ...
    expect(screen.getByText(/etapa 3 de 3/i)).toBeInTheDocument();
    expect(screen.queryByTestId('provider-map')).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /fornecedor do edu/i })).not.toBeInTheDocument();
    await waitFor(() => expect(screen.getByRole('button', { name: /finalizar/i })).toBeEnabled());
  });
  ...
  it('lets the dentist complement the shared intake while customer answers stay read-only', async () => {
    ...
    await screen.findByTestId('dental-anamnesis-record');
    expect(screen.queryByLabelText(/resumo da avalia.*inicial \/ anamnese/i)).not.toBeInTheDocument();
    expect(screen.getAllByText('Resumo clínico completo.').length).toBeGreaterThan(0);
  ...
  it('does not reuse a submitted dentist workflow cache when the current dentist still has a pending complement', async () => {
    const queryClient = createTestQueryClient();

    queryClient.setQueryData(['biteplaner', 'orders', 'BP-DEMO-004', 'workflow-forms'], {
      forms: [reviewedSharedIntake()],
    });
    ...
    expect(screen.getByRole('button', { name: /salvar complemento do dentista/i })).toBeInTheDocument();
    expect(screen.queryByText(/complemento do dentista indispon/i)).not.toBeInTheDocument();
  });
  ...
  it('lets the dentist download the anamnesis PDF before completing production', async () => {
    configureApiGet();
    ...
    expect(MockPdfWorker.instances[0]?.postMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        draft: expect.objectContaining({ anamnesisSummary: expect.any(String) }),
      })
    );
  ...
  it('removes the standalone professional observations card from the anamnesis PDF', () => {
    ...
    expect(pdfSource).not.toContain('OBSERVAÇÕES PROFISSIONAIS');
    expect(pdfSource).not.toContain('<Section number={9} title=\"OBSERVAÇÕES PROFISSIONAIS\">');
  });
