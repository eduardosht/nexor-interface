...
describe('biteplanerOrderCompletion api', () => {
  ...
  it('creates and confirms slot uploads on commerce completion routes', async () => {
    ...
    await createCompletionAttachmentUploadIntent('order-1', 'upper_scan', {
      fileName: 'superior.stl',
      mimeType: 'model/stl',
      sizeBytes: 2048,
    }, 'token');
    await confirmCompletionAttachmentUpload('order-1', 'upper_scan', {
      uploadId: 'upload_1',
      objectKey: 'biteplaner/production-scans/profile/orders/order-1/tmp/upper_scan/upload_1/superior.stl',
      fileName: 'superior.stl',
      mimeType: 'model/stl',
      sizeBytes: 2048,
    }, 'token');
    ...
    expect(api.post).toHaveBeenNthCalledWith(1, '/v1/commerce/biteplaner/orders/order-1/attachments/upper_scan/upload-intent', expect.any(Object), 'token');
    expect(api.post).toHaveBeenNthCalledWith(2, '/v1/commerce/biteplaner/orders/order-1/attachments/upper_scan/confirm', expect.any(Object), 'token');
  ...
  it('uploads files to the presigned S3 URL before confirming the slot', async () => {
    vi.mocked(api.post)
      ...
      .mockResolvedValueOnce({ fileRef: { id: 'attachment-1', slotKey: 'upper_scan' } });
    const fetchImpl = vi.fn(async () => new Response(null, { status: 200 }));
    const file = new File(['solid scan'], 'superior.stl', { type: 'model/stl' });

    await uploadCompletionSlotFile({ orderId: 'order-1', slotKey: 'upper_scan', file, token: 'token', fetchImpl });
    ...
    expect(api.post).toHaveBeenLastCalledWith('/v1/commerce/biteplaner/orders/order-1/attachments/upper_scan/confirm', expect.objectContaining({
      uploadId: 'upload_1',
      objectKey: 'object-key',
      fileName: 'superior.stl',
    }), 'token');
