...
- [ ] Write a failing Vitest test that verifies the public image manifest exposes AVIF/WebP URLs for Home and Biteplaner assets.
- [ ] Add `sharp` and an `optimize:images` npm script.
- [ ] Implement `scripts/optimize-public-images.mjs` with explicit source files, output widths, and formats.
- [ ] Run `npm run optimize:images` and commit generated assets under `src/assets/generated/public`.
- [ ] Run the manifest test and verify it passes.
...
- [ ] Replace original PNG imports on Home and `/biteplaner` with manifest URLs.
- [ ] Use responsive `image-set(...)` for CSS backgrounds.
- [ ] Add explicit dimensions/loading hints to image elements.
- [ ] Keep the Home hero video source unchanged, but add an optimized poster image.
...
- [ ] Write a failing test that verifies public page imports are lazy.
- [ ] Convert public page imports to `React.lazy`.
- [ ] Wrap public route elements in `Suspense`.
- [ ] Run route tests.
...
- [ ] Run `npm run build`.
- [ ] Compare asset/chunk sizes from build output against the original baseline.
